...
public class GuideMask : MaskableGraphic, ICanvasRaycastFilter
{
    ...
    public void Play(RectTransform target, int step)
    {
        gameObject.SetActive(true);
        for(int i = 1; i <= 6; i++)
        {
            transform.Find(string.Format("guide_{0}", i)).gameObject.SetActive(false);
        }
        transform.Find(string.Format("guide_{0}", step)).gameObject.SetActive(true);
